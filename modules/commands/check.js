...
module.exports.config = {
  ...
  version: "1.0.1",
  hasPermssion: 0,
  credits: "DungUwU && Nghĩa (Firebase by AI)",
...
module.exports.handleEvent = async function({ api, event }) {
  try {
    ...
    const today = moment.tz("Asia/Ho_Chi_Minh").day();
    let threadData = await firebase.get('checktt', threadID);
    if (!threadData) {
      threadData = {
        total: [],
        week: [],
    ...
    for (let user of UserIDs) {
      ...
      if (!threadData.last.week.find(item => item.id == user)) threadData.last.week.push({ id: user, count: 0 });
      if (!threadData.last.day.find(item => item.id == user)) threadData.last.day.push({ id: user, count: 0 });
      if (!threadData.total.find(item => item.id == user)) threadData.total.push({ id: user, count: 0 });
      if (!threadData.week.find(item => item.id == user)) threadData.week.push({ id: user, count: 0 });
      if (!threadData.day.find(item => item.id == user)) threadData.day.push({ id: user, count: 0 });
    }
    const userData_week_index = threadData.week.findIndex(e => e.id == senderID);
    const userData_day_index = threadData.day.findIndex(e => e.id == senderID);
    const userData_total_index = threadData.total.findIndex(e => e.id == senderID);
    if (userData_total_index == -1) threadData.total.push({ id: senderID, count: 1 });
    else threadData.total[userData_total_index].count++;
    if (userData_week_index == -1) threadData.week.push({ id: senderID, count: 1 });
    else threadData.week[userData_week_index].count++;
    if (userData_day_index == -1) threadData.day.push({ id: senderID, count: 1 });
    else threadData.day[userData_day_index].count++;
    // Lọc user không còn trong nhóm
    let p = participantIDs;
    if (!!p && p.length > 0) {
      p = p.map($=>$+'');
      ['day','week','total'].forEach(t=>threadData[t] = threadData[t].filter($=>p.includes($.id+'')));
    }
    await firebase.set('checktt', threadID, threadData);
  } catch(e){}
};
...
module.exports.run = async function({ api, event, args }) {
  ...
  let threadData = await firebase.get('checktt', threadID);
  if (!threadData) return api.sendMessage("⚠️ Chưa có dữ liệu", threadID);
