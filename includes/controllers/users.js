...
module.exports = function () {
	return {
		...
		async getNameUser(id) {
			try {
				const data = await firebase.get('users', id);
				return data && data.name ? data.name : 'Người dùng facebook';
			} catch {
		...
		async getAll() {
			try {
				return await firebase.getAll('users');
			} catch (error) {
				console.error(error);
		...
		async getData(userID) {
			try {
				return await firebase.get('users', userID) || false;
			} catch (error) {
				console.error(error);
				throw new Error(error);
			}
		...
		async setData(userID, options = {}) {
			...
			try {
				await firebase.set('users', userID, options);
				return true;
			} catch (error) {
				console.error(error);
				throw new Error(error);
		...
		async delData(userID) {
			try {
				await firebase.del('users', userID);
				return true;
			} catch (error) {
				console.error(error);
				throw new Error(error);
		...
		async createData(userID, defaults = {}) {
			...
			try {
				await firebase.set('users', userID, defaults);
				return true;
			} catch (error) {
				console.error(error);
				throw new Error(error);
